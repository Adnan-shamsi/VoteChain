...
class Blockchain {
  ...
  newCommerNeverExistedInChainBefore(address) {
    for (let i = this.chain.length - 1; i > 0; i--) {
      const { newCommerTransactions } = this.chain[i].data.transactions;
  ...
  static validTransactionData({ chain }) {
    ...
    for (let i = 1; i < chain.length; i++) {
      ...
      if (
        !Transaction.validateWinnerTransactions({ votingTransactions, winners })
      ) {
      ...
      for (let transaction of newCommerTransactions) {
        const addressOfNewCommer = transaction[0];
        const signature = transaction[1];

        if (!verifyNewCommerSignature(addressOfNewCommer, signature)) {
          console.error("NewCommerSignature miss match");
          return false;
        }
        ...
        if (newCommerSet.has(addressOfNewCommer)) {
          ...
          return false;
			...
			newCommerSet.add(addressOfNewCommer);
          if (currentBalance[addressOfNewCommer]) {
            currentBalance[addressOfNewCommer] += NEW_COMMER_BALANCE;
          }else{
			currentBalance[addressOfNewCommer] = STARTING_BALANCE + NEW_COMMER_BALANCE;
		  }
        }
      }
    }
